...
| 口径负责人 | 开发者 B |
| 走查会签 | 开发者 A（平台框架开发，已签）、开发者 B（引擎skill开发，已签） |
| 状态 | M0 冻结 |

**状态说明**：本文所有口径经 A、B 共同走查并在上表签字（2026-08-29），已整体转为**冻结**，`formula_version` 保持 `"0.1.0"`。未冻结口径不得写入默认 `AnalysisResult`，只能以实验输出存在（`experimental/` 路径，显式标注非默认）。冻结后的任何变更必须执行第 11 节流程。

| 走查方 | 签字 | 日期 |
|---|---|---|
| 开发者 A | 平台框架开发 | 2026-08-29 |
| 开发者 B | 引擎skill开发 | 2026-08-29 |
