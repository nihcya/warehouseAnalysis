...
- **误差求和口径（M2 实现决策，需 A/B 复核）**：§8.2 的 ``n`` 在训练窗口充足
  时即预测窗口期数。当训练窗口过短导致预测窗口早期期次无前 4 周可用（如
  ``split_date`` 恰等于 ``start_date`` 时第 1 周）而无法产生预测时，这些期次
  **从分子与分母中一并剔除**——否则分子漏项、分母仍计入，会使 MAPE/WAPE/
  MAE/RMSE 相互不同口径。此时 ``n``（以及 WAPE 的 ``Σd_t``）取可预测期数；
