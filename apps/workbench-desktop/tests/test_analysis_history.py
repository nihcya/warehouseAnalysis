...
def test_history_tab_lists_runs_and_reloads_result(
    ...
    first_run_id = page.result_table.item(0, 0).text()
    page.run_button.click()
    second_run_id = page.result_table.item(0, 0).text()
    ...
    assert page.result_table.item(0, 0).text() == second_run_id
    stored = store.get(second_run_id)
    assert stored is not None
    assert stored.run_id == page.result_table.item(0, 0).text()
    assert stored.engine_version == page.result_table.item(0, 1).text()
    assert stored.formula_version == page.result_table.item(0, 2).text()
    ...
    assert page.tabs.currentIndex() == RUN_TAB_INDEX
    assert page.result_table.item(0, 0).text() == first_run_id
